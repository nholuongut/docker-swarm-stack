...
docker-swarm-stack includes a modern DevOps workflow for monitoring and alerting about your containerized applications running within Docker swarms, including monitoring and alerting of the cluster health itself as well as the health of your own applications. docker-swarm-stack installs and updates [Prometheus](https://github.com/prometheus/prometheus/blob/master/README.md) + [Grafana](https://grafana.com) + [Alertmanager](https://github.com/prometheus/alertmanager/blob/master/README.md). docker-swarm-stack also provides an optional installation of [Portworx](https://portworx.com) for persistent storage for containers such as databases that need storage that can move to another Docker swarm node instantly, or bring your own persistent storage layer for Docker (e.g. [RexRay](https://github.com/rexray/rexray), or use local host volumes and add placement constraints to _[docker-compose.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/docker-compose.yml)_) 
...
![](https://raw.githubusercontent.com/nholuongut/docker-swarm-stack/master/documentation/screens/docker-swarm-stack-diagram.png "docker-swarm-stack architecture")
...
- Tunes EL7 [sysctls](https://raw.githubusercontent.com/nholuongut/docker-swarm-stack/master/ansible/playbooks/includes/tasks/sysctl_el7.yml) for optimal network performance
...
- _(DevOps: docker-swarm-stack)_ Configures and deploys the docker-swarm-stack tool chain, including Prometheus and Pushgateway, redundant Alertmanager instances, Grafana, karma, and Portainer containers, and also installs NetData under systemd on each host. All tools are secured using HTTPS by a Caddy reverse-proxy.

Optional

- [Errbot](https://github.com/docker-swarm-stack/errbot-docker) - Connect alerts to social rooms to not already natively supported by Alertmanager
- [InfluxDB](https://github.com/docker-swarm-stack/influxdb) - [deprecated] Useful for longer-term Prometheus storage/retrieval in certain workloads
- [Loki](https://github.com/docker-swarm-stack/loki) - Like Prometheus but for logs
- [TeamPass](https://github.com/docker-swarm-stack/teampass) - Secure collaborative team management for shared credentials
- [Trickster](https://github.com/docker-swarm-stack/trickster) - In-memory Prometheus data cache for frequently re-requested tsdb blocks (accelerator for Grafana if you have popular dashboards)
- [VictoriaMetrics](https://github.com/docker-swarm-stack/victoria-metrics) - Fast, cost-effective and scalable time series database, long-term remote storage for Prometheus
...
![](https://raw.githubusercontent.com/nholuongut/docker-swarm-stack/master/documentation/screens/Cluster_Nodes.png)
...
There is a [docker-compose-singlebox.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/docker-compose-singlebox.yml) stack that can be used to evaluate docker-swarm-stack on a single Docker swarm host without requiring etcd and Portworx or other persistent storage. Please see the INSTALLATION section for instructions on installing this _singlebox_ version of docker-swarm-stack.

3 or more Enterprise Linux 7 (RHEL 7/CentOS 7) hosts _(baremetal / VM or a combination)_, with each contributing (1) or more additional virtual or physical _unused_ block devices or partitions to the Portworx storage cluster that docker-swarm-stack will install. _More devices usually equals better performance_. If bringing your own existing Docker swarm, 17.12.0+ is required by the docker-swarm-stack compose file (version 3.5). docker-swarm-stack will install or upgrade all defined _[swarm]_ nodes to the latest stable version of Docker and will attempt to join all _[swarm]_ hosts in the cluster to the _SWARMJOIN_ defined host. You can change the Docker version installed, including using bleeding-edge repos if desired, by altering the docker.yml playbook.

Using the free [Portworx PX-Developer](https://github.com/portworx/px-dev) version by default, docker-swarm-stack will install a storage cluster for each set of (3) hosts added to the cluster, which must provide a minimum 40GB (needed by docker-swarm-stack) up to the Portworx developer version limits of 1TB of persistent storage for up to _40_ volumes across those 3 nodes. Adding 1TB drives/partitions from each node would be optimal to fill out the 1TB of replicated space. You can contribute unused block device(s) or partition(s), adding more smaller NMVe/SSDs on bare-metal or cloud-provider high-IOPS block devices would provide Portworx faster storage, but Portworx functions even across 3 VMs on the same machine each contributing storage from a single shared USB2 NAS, so scale your storage depending on your expected persistent-storage workloads. Block devices or partitions larger than 1TB can be contributed, but only 1TB of persistent storage will be available without licensing the PX-Enterprise version.
...
- _For manual docker-swarm-stack installation instructions, see [Manual docker-swarm-stack installation.md](https://github.com/nholuongut/docker-swarm-stack/blob/master/documentation/Manual%20docker-swarm-stack%20installation.md)_

- _When installing behind a required web proxy, see [Working with docker-swarm-stack behind a web proxy.md](https://github.com/nholuongut/docker-swarm-stack/blob/master/documentation/Working%20with%20docker-swarm-stack%20behind%20a%20web%20proxy.md)_

- _Instructions for updating docker-swarm-stack are available at [Updating docker-swarm-stack.md](https://github.com/nholuongut/docker-swarm-stack/blob/master/documentation/Updating%20docker-swarm-stack.md)_

- _To deploy and monitor your own applications on the cluster, see [Adding your own applications to monitoring.md](https://github.com/nholuongut/docker-swarm-stack/blob/master/documentation/Adding%20your%20own%20applications%20to%20monitoring.md)_

- _To manually push ephemeral or batch metrics into Prometheus, see [Using Pushgateway.md](https://github.com/nholuongut/docker-swarm-stack/blob/master/documentation/Using%20Pushgateway.md)_

- _For reference on what docker-swarm-stack configures when enabling LDAP, see [Using LDAP.md](https://github.com/nholuongut/docker-swarm-stack/blob/master/documentation/Using%20LDAP.md)_

- _Some basic commands for working with docker-swarm-stack and Portworx storage are noted in [Notes.md](https://github.com/nholuongut/docker-swarm-stack/blob/master/documentation/Notes.md)_

- _Open an issue. [How do I use this project?](https://github.com/nholuongut/docker-swarm-stack/issues/1)_ 
...
# git clone https://github.com/nholuongut/docker-swarm-stack

# rsync -aq --exclude=.git --exclude=.gitignore docker-swarm-stack/ localdocker-swarm-stack/

# cd localdocker-swarm-stack/ansible
```

There is a [docker-compose-singlebox.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/docker-compose-singlebox.yml) stack that can be used to evaluate docker-swarm-stack on a single Docker swarm host without requiring etcd and Portworx or other persistent storage. This stack will save persistent named volumes to the single swarm host instead. Please see the file for installation instructions and _skip all other steps below_.

Edit these (4) files: | |
---- | - |
[clusters/docker-swarm-stack](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/clusters/docker-swarm-stack) | _Configure all of your cluster nodes and storage devices_ |
[alertmanager/conf/alertmanager.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/alertmanager/conf/alertmanager.yml) | _Optional: Configure where the Alertmanagers send notifications_ |
[roles/docker-swarm-stack/files/etc/docker-swarm-stack_fw/rules/cluster.rules](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/roles/docker-swarm-stack/files/etc/docker-swarm-stack_fw/rules/cluster.rules) | _Used to permit traffic to the hosts themselves_ |
[roles/docker-swarm-stack/files/etc/docker-swarm-stack_fw/rules/docker.rules](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/roles/docker-swarm-stack/files/etc/docker-swarm-stack_fw/rules/docker.rules) | _Used to limit access to Docker service ports_ |
...
After execution of the docker-swarm-stack.yml playbook, you'll log into most of the tools as 'admin' and the ADMIN_PASSWORD set in [ansible/clusters/docker-swarm-stack](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/clusters/docker-swarm-stack). You can update the ADMIN_PASSWORD later by executing _docker stack rm docker-swarm-stack_ (persistent data volumes will be preserved) and then re-deploy the docker-swarm-stack [docker-compose.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/docker-compose.yml)

Instances such as Grafana and Portainer will save credential configuration in their respective persistent data volumes. These volumes can be manually removed if required and would be automatically re-initialized the next time the docker-swarm-stack compose file is deployed. You would lose any historical information such as metrics if you choose to initialize an application by executing _docker volume rm docker-swarm-stack_volumename_ before re-running the docker-swarm-stack.yml playbook.

---

ansible-playbook -i clusters/docker-swarm-stack [playbooks/firewall.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/playbooks/firewall.yml) -k
...
ansible-playbook -i clusters/docker-swarm-stack [playbooks/docker.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/playbooks/docker.yml) -k
...
ansible-playbook -i clusters/docker-swarm-stack [playbooks/etcd.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/playbooks/etcd.yml) -k
...
ansible-playbook -i clusters/docker-swarm-stack [playbooks/portworx.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/playbooks/portworx.yml) -k

* _(optional)_ Installs Portworx in groups of 3 nodes each. If you are instead bringing your own persistent storage, be sure to update the pxd driver in [docker-compose.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/docker-compose.yml). Add new groups of 3 hosts later as your cluster grows. You can add 1 or more additional Docker swarm hosts in the [swarm] group but not in the [portworx] group; these extra hosts can absorb load across the cluster from containers not requiring persistent storage.

---

ansible-playbook -i clusters/docker-swarm-stack [playbooks/docker-swarm-stack.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/playbooks/docker-swarm-stack.yml) -k

* _This deploys or redeploys the docker-swarm-stack DevOps monitoring stack to the Docker swarm cluster. This includes installing or updating NetData on each node in order for Prometheus to collect metrics from it._

### Updating

If you make changes to the local service configuration files in /usr/local/src/localdocker-swarm-stack/, such as altering Prometheus configuation or rules, you can run the [up](https://github.com/nholuongut/docker-swarm-stack/blob/master/up) script which will cause swarm to update only the services where configuration changes are detected. There is a similar script [up-singlebox](https://github.com/nholuongut/docker-swarm-stack/blob/master/up-singlebox) for these users as well.
...
To attach your own applications into Prometheus monitoring, you'll need to make sure you deploy your services to attach to the docker-swarm-stack monitoring network in your own docker-compose files. See [docker-swarm-stack/errbot-docker/docker-compose-docker-swarm-stack.yml](https://github.com/docker-swarm-stack/errbot-docker/blob/master/docker-compose-docker-swarm-stack.yml) for an example Docker compose service attaching to docker-swarm-stack_net to be monitored:
...
You'll need to expose your metrics in a [Prometheus-compatible](https://www.youtube.com/watch?v=Zk09Mbu0YQk) format on an HTTP port (traffic on the docker-swarm-stack_net overlay network is encrypted), and add the following labels to your service so that Prometheus will quickly start to scrape it. See [nholuongut/docker-swarm-stack/docker-compose.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/docker-compose.yml) for examples of services announcing they wish to be monitored by Prometheus:
...
You can configure the routes and receivers for the docker-swarm-stack Alertmanager instances by editing /usr/local/src/localdocker-swarm-stack/[alertmanager/conf/alertmanager.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/alertmanager/conf/alertmanager.yml)
...
One option would be to configure a bot listening for alerts from Alertmanager instances (you could use it also as an alerting target for your code as well). Upon receiving alerts via a web server port, the bot would then relay them to an instant-message destination. docker-swarm-stack builds a Docker image of Errbot, one of many bot programs that can provide a conduit between receiving a webhook, processing the received data, and then connecting to something else (in this case, instant-messaging networks) and relaying the message to a recipient or a room's occupants. If this sounds like the option for you, please see the following project:

[https://github.com/docker-swarm-stack/errbot-docker](https://github.com/docker-swarm-stack/errbot-docker)

### Logging

There is an adjunct [https://github.com/docker-swarm-stack/loki](https://github.com/docker-swarm-stack/loki) project that can be used to quickly turn up an instance of Grafana Loki, which your docker-swarm-stack or other Grafana instance can configure as a data source. By default, this project will feed the stdout and stderr of all of your Docker swarm containers into Loki (using the loki-docker-driver Docker plugin on each node). Loki can also be fed directly by your own applications, via Fluentd / Fluent Bit, or via other logging facilities.
...
Below is mainly for documentation. After installing docker-swarm-stack, just connect to https://swarmhost of any Docker swarm node and authenticate with your ADMIN_PASSWORD to view the links:
...
-------- | ----- | --------------
[Caddy](https://caddyserver.com/) | Reverse proxy - see above for URLs | Source:[https://github.com/docker-swarm-stack/caddy](https://github.com/docker-swarm-stack/caddy)<br>Image:[https://hub.docker.com/r/docker-swarm-stack/caddy](https://hub.docker.com/r/docker-swarm-stack/caddy) no-stats
...
[Fail2ban](https://www.fail2ban.org) | Brute-force prevention | [ansible/playbooks/firewall.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/playbooks/firewall.yml)
[iptables](https://en.wikipedia.org/wiki/Iptables) | Firewall management | [ansible/playbooks/firewall.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/playbooks/firewall.yml)
...
[Docker Swarm](https://docs.docker.com/engine/swarm/) | Docker Node IP:<br>http://swarmhost:9323/metrics | [ansible/playbooks/docker.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/playbooks/docker.yml)
[etcd3](https://github.com/etcd-io/etcd) | Docker Node IP:<br>http://swarmhost:2379/metrics | [ansible/playbooks/etcd.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/playbooks/etcd.yml)
...
[NetData](https://my-netdata.io/) | Docker Node IP:<br>http://swarmhost:19999/api/v1/allmetrics | [ansible/playbooks/docker-swarm-stack.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/playbooks/docker-swarm-stack.yml) latest
...
[Portworx](https://portworx.com) | Docker Node IP:<br>http://swarmhost:9001/metrics | [ansible/playbooks/portworx.yml](https://github.com/nholuongut/docker-swarm-stack/blob/master/ansible/playbooks/portworx.yml) px-dev or px-enterprise 2.4.0
